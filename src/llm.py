...
def create_calibration_dataset(vocab_size=50257, num_samples=32, seq_len=32):
    # Random tokens in [0 .. vocab_size)
    input_ids = torch.randint(low=0, high=vocab_size, size=(num_samples, seq_len), dtype=torch.long)
...
weight_scales = collect_linear_weight_scales(model, prefix="")
...
for name, module in model.named_modules():
    if isinstance(module, nn.Linear):
        def hook_fn(m, inp, out, modname=name):
            tensor = out if isinstance(out, torch.Tensor) else out[0]
            min_val = tensor.min().item()
            max_val = tensor.max().item()
            if modname not in act_mins:
                act_mins[modname] = min_val
                act_maxs[modname] = max_val
            else:
                act_mins[modname] = min(min_val, act_mins[modname])
                act_maxs[modname] = max(max_val, act_maxs[modname])
...
activation_scales = {}
for linear_name in act_mins.keys():
    min_val = act_mins[linear_name]
    max_val = act_maxs[linear_name]
    scale_val = compute_activation_scale_symmetric(min_val, max_val)
    activation_scales[linear_name] = scale_val

replace_linear_with_quantlinear(model, weight_scales, activation_scales, prefix="")
model.to("cuda")
